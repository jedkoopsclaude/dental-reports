...
class Labfees extends CI_Controller {
	...
	public function index($date = '')
	{
		...
		// Get Jerry's Collections

		$jerryID = 44;
		//Jerry's Patient collections 
		$query = $opendentaldb->query("SELECT SUM(ps.SplitAmt) AS pattotal FROM paysplit ps ".
				" WHERE ps.ProvNum=$jerryID AND ps.DatePay BETWEEN '$startDate' AND '$endDate'");
		$jfpRow = $query->row();
		...
		$query = $opendentaldb->query("SELECT SUM(cp.InsPayAmt) AS instotal FROM claimpayment cy ".
				" INNER JOIN claimproc cp ON cp.ClaimPaymentNum=cy.ClaimPaymentNum ".
			 	" WHERE cp.ProvNum=$jerryID AND cy.CheckDate  BETWEEN '$startDate' AND '$endDate'");
		$jfiRow = $query->row();
		$jfIns = '$'.number_format($jfiRow->instotal, 2);
		
		// Calc Jerry total collections
		$jfTotal = $jfpRow->pattotal + $jfiRow->instotal;
		$prettyjfTotal = '$'.number_format( $jfTotal, 2 );
		$html .= "<div id='jerrytotal'><span class='provname'>Jerry Collections:</span><span class='accounting'>$prettyjfTotal</span></div>";
		$thirtythree = $jfTotal * 0.33;
		$pretty33 = '$'.number_format( $thirtythree, 2);
		$html .= "<div id='jerry33' class='row'><span class='provname'>33%:</span><span class='accounting'>$pretty33</span></div>";
		...
		*/
		...
		$query = $opendentaldb->query("SELECT l.DateTimeRecd,CONCAT(p.FName,' ',p.LName) as Name,".
				...
				" LEFT JOIN laboratory lab ON lab.LaboratoryNum = l.LaboratoryNum ".
				" LEFT JOIN appointment a ON a.AptNum = l.AptNum ".
				" WHERE l.ProvNum=$jerryID AND datetimerecd BETWEEN '$startDate' AND '$endDate' ORDER BY datetimerecd");
		$labCases = array();
		// show header columns
		...
		foreach ($query->result() as $row)
		{
			...
			foreach ($labCaseCols as $key => $value) {
				$labhtml .= "<span class='labcol $key'>" . $row->$key . "</span>\n";
			}
			$labhtml .= "</div>";
			$totalFees += $row->LabFee;
		}
		$labTotal = '$'.number_format( $totalFees, 2);
		$twenty5 = $totalFees * 0.25;
		...
		$html .= "<div id='lab25'><span class='provname'>25%:</span><span class='accounting'>$labPercent</span></div>";

		// Now calculate Jerry's paycheck
		...
		$query = $opendentaldb->query("SELECT COUNT(*) AS 'Total' ".
			...
			" WHERE pl.ProvNum=$jerryID AND pl.ProcDate BETWEEN '$startDate' AND '$endDate'".
